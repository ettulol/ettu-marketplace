...
Use the host's plugin manager to replace the whole bundle, preserving deliberate endpoint customizations, connection identity, account and credentials. Version 0.12.0 changes this distribution’s default endpoint from localhost to `https://ettu.lol/mcp`. On an authorized update of the official default installation, apply the new bundled endpoint and complete production OAuth when prompted; local accounts/data do not migrate. If a user deliberately configured a custom server, preserve it unless switching is also authorized. Registered ChatGPT connections require their host’s connection controls to update the server URL; a bundle update alone does not do that. Do not hand-edit installed cache files, replace only a skill, remove the marketplace, or overwrite local connection customizations as an update shortcut. If a new release changes connection requirements, explain that before applying it.
