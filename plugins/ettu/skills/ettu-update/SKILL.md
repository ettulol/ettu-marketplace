...
Use the host's plugin manager to replace the whole bundle, preserving the configured endpoint, connection identity, account and credentials. Do not hand-edit installed cache files, replace only a skill, remove the marketplace, or overwrite local connection customizations as an update shortcut. If a new release changes connection requirements, explain that before applying it.
