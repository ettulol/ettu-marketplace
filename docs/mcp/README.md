...
The website’s `/terms`, `/privacy` and `/copyright` pages contain static policy text. The Home footer’s **Contact Us** link opens an email to `ettu@sutrocloud.com`. Policy copy, contact links, installed-app launch screens and shared-link branding do not add product-data operations or change MCP account permissions.
