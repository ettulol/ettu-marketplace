...
- `regenerate_character` takes a selected ready `version` (including currently or previously published) or a failed unpublished version, the current `expected_version`, and a UUID `request_key`. The owner must request fresh artwork. It copies that version's definition/interview exactly into a new private version with current models/style rules and draws fresh artwork even when the definition is unchanged. The currently published portrait anchors identity; the reference does not guarantee identical appearance with updated models. Source snapshots, errors and content-reviewed previews follow normal retention. Content-rejected and archived targets are blocked; queued/generating work blocks a new attempt, while replaying an accepted request returns its existing job. Revise content rejections through `update_character`. It never changes publication; `publish_character` is still explicit. Website owners use **Versions → Generate new version** for ready artwork and **Regenerate character** for failed drafts.
...
Generate fresh artwork from a ready version (including currently or previously published) or a failed unpublished version of your character as a NEW private version, with the same definition and interview and the current image models and artwork recipe. Read get_character/list_character_versions first. Pass the selected source version and current expected_version. This intentionally redraws artwork even when its definition is unchanged; it does not reuse the old animation. A published character uses its currently published portrait for identity continuity. Preserves the selected source and any inspectable frames under normal 20-version retention; the existing publication stays in place. Identity references guide continuity but cannot guarantee identical appearance with updated models. Website owners can choose Generate new version under Versions for ready artwork, or Regenerate character for a failed draft. Only use on the owner's request; this queues paid generation and never publishes. A content rejection must be revised with update_character instead. An active queued/generating version blocks another regeneration. Use a fresh request_key per intended generation; reuse the SAME key after a timeout or lost response, even if expected_version has changed. The receipt survives version deletion/pruning: retained=false means that earlier attempt was removed, not that another generation started.
